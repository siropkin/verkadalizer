...
export async function generateRequestIdFromImage(imageUrl) {
  ...
  // Load dietary preference AND image style settings
  ...
  const styleId = stored.imageStyle || 'verkada-classic';
  ...
  const requestId = `img_${Math.abs(hash).toString(36)}_${preferenceId}_${styleId}`;
  return requestId;
}
...
export async function saveGeneratedImageToStorage(requestId, generatedSrc) {
  try {
    ...
    logWarn('lib', 'storage', 'Failed to save generated image', error.message);
...
export async function loadSavedImageFromStorage(requestId) {
  try {
    ...
    logWarn('lib', 'storage', 'Failed to load saved image', error.message);
    throw error;
  }
}
...
export async function cleanupOldSavedImages() {
  try {
    ...
    logWarn('lib', 'storage', 'Failed to cleanup old saved images', error.message);
